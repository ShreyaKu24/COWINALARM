...
async function onload()
{
    ...
    for (let i = 0; i < states.length; i++)// option wala is html content to form dropdown option
        statesSelect.innerHTML = statesSelect.innerHTML+  '<option class="option" value="' + states[i].stateId + '">' + states[i].stateName + '</option>'

    statesSelect.addEventListener('change', () => // arrow function used here. can be normal function , addeventlistener func is used here for adding listener to events passed in that like change event here
    {
        resetFilter()
        fillDistricts()

        window.history.pushState('', '', relativePath + "?districtId=" + districtsSelect.value)//change the urk of page on selectimg a particular district
        previousDetailsHtml = ''
...
async function refreshTable(renderFilter)
{
    ...
    let todayString = getTodayString()//give today's date
    ...
    try
    {
        response = await fetch(endpoint)//fetch api data
    }
    catch
    {
...
function main(data, renderFilter)
{
    ...
    {
        let sessionCountMap = new Map()//map is like the object set the name center..  
        centresData.map( center => // map function is like for loop. read map() function
             center.sessions.map(session =>
...
function renderFiltersInputs(data)
{
    ...
    //js uses json.parse to conver json into objects
}
...
function playAlert(newCentres)
{
    ...
    let tone = new Audio(relativePath + '../assets_beep.mp3')
